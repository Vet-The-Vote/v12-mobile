...
This project enables you to create a V12 Mobile Phone, which is a privacy phone.  A V12 Mobile Phone comes with all Google services disabled, so that Google's ad network cannot be used to spy on your location nor gather other sensitive personal data without your knowledge.

# Install Graphene OS on new Pixel 8

To install Graphene OS on a new Pixel 8 phone, first download this repository 
...
## Install Instructions for Pixel 8

1. Plugin Pixel 8 to a USB-C on the back of a computer, which is running Debian 11 (bullseye).  Note, do not plug the Pixel 8 into a USB port on the front of the computer.  See the official [Graphene OS installation prerequisites](https://grapheneos.org/install/cli#prerequisites) for more details.
2. Enable "Developer Mode" on the Pixel 8 by doing as follows:
    - From the home screen, swipe up on the screen.
    - Tap on `Settings -> System -> About Phone`, then scroll all the way to the bottom.
    - Tap on `Build number` seven times, making sure the Pixel 8 shows the messages "You are now a developer".
3. Enable OEM Unlocking on the Pixel 8 by doing the following:
    - From the home screen, swipe up on the screen.
...
5. Power off the Pixel 8, by pressing both buttons at the same time: `Power` & `Volume Up`.
6. Hold down `Volume Down` while powering back up the Pixel 8.  Continue holding `Volume Down` until it boots up into the bootloader interface.
7. After the Pixel 8 boots up into the bootloader, execute the following from the command line: `fastboot flashing unlock`.
8. On the Pixel 8, confirm to unlock the bootloader by doing the following:
...
10. After the script `download-and-verify-Pixel-8.sh` has completed, lock the bootloader on the Pixel 8 by doing the following:
    ...
    - On the Pixel 8:
        ...
        - Press the `Power` button.
        - The Pixel 8 will reboot and go back into the bootloader screen.
11. Reboot into Graphene OS by doing the following:
        - Boot Press the `Power` button on the Pixel 8 phone.  The Pixel 8 should reboot into Graphene OS after 1-2 minutes.
12. Verify the following boot key hash is displayed with a yellow notice when the Pixel 8 first powers on.
    - `cd7479653aa88208f9f03034810ef9b7b0af8a9d41e2000e458ac403a2acb233`
13. Disable OEM unlocking on the Pixel 8 by doing the following:
    ...
    - Toggle off `Use developer options`, then tap `Restart`.  The Pixel 8 should reboot.

After executing the installation steps in this section, your Pixel 8 should 
...
1. Install the `F-Droid` app store by opening the following link on the Pixel 8:
...
At this point the F-Droid app store should now be installed on the Pixel 8.  
...
At this point the Briar app should now be installed on the Pixel 8.  
...
5. Tap `CREATE ACCOUNT` to create your Briar account on your Pixel 8.
